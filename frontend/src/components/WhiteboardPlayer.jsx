...
// --- 2. COMPONENTE DE ELEMENTO LATEX  ---
const ElementoLatex = ({ data, state, onClick, stepIndex, isCurrentStep }) => {
  ...
  return (
    ...
    >
      ...
      <span
        className={`whitespace-nowrap pointer-events-none transition-all duration-300 ${isCurrentStep ? "font-medium" : "font-normal"} text-lg md:text-2xl`}
      >
...
const WhiteboardPlayer = ({
    ...
    scene.insts.forEach((inst, stepIndex) => {
      if (inst.tgs) {
        inst.tgs.forEach((tg) => {
          ...
          if (map[parseInt(idStr)] === undefined)
  ...
  const layoutData = useMemo(() => {
    if (!scene || !scene.cont)
      return { elements: [], boxes: [], totalHeight: 800 };
    const items = [];
    let currentBox = null;
    scene.cont.forEach((el, idx) => {
      ...
      if (el.apart === "start" || el.apart === "start-end") {
        currentBox = {
          ...
          minY: el.y,
          maxY: el.y,
          collapsed: !!collapsedBoxes[`box-${idx}`],
        };
    ...
    items.forEach((item) => {
      if (item.type === "single") {
        ...
        if (!item.collapsed) {
          ...
          const boxHeight =
            item.maxY - item.minY + PADDING_TOP + PADDING_BOTTOM;
          visualBoxes.push({
            id: item.id,
            y: boxTop,
            x: 80,
    ...
    return {
      ...
      boxes: visualBoxes,
      totalHeight: maxGlobalY + 300,
    };
  }, [scene, collapsedBoxes]);

  // --- EFECTOS ---
  useEffect(() => {
    if (currentStepIdx > maxStepReached) setMaxStepReached(currentStepIdx);
  }, [currentStepIdx]);
  useEffect(() => {
    if (
      requestedStep !== null &&
      requestedStep !== undefined &&
      requestedStep !== currentStepIdx
    )
    ...
    if (currentInst.tgs) {
      currentInst.tgs.forEach((tgObj) => {
        if (tgObj.ac === "resalt") {
          ...
          if (newState[parseInt(idStr)])
            newState[parseInt(idStr)].highlights.push({
              start,
              end,
              color: tgObj.color || "#00ff66",
            });
        }
      });
    }
  ...
  return (
    <div
      className={`flex flex-col h-full bg-[#0a0a0a] overflow-hidden shadow-2xl relative font-sans select-none ${isFullscreen ? "rounded-none border-0" : "rounded-xl border border-neutral-800"}`}
    >
      {/* HEADER */}
      <div className="bg-[#111] border-b border-neutral-800 px-6 py-4 flex justify-between items-center z-20 shrink-0">
        <h2 className="text-lg md:text-xl font-bold text-[#00ff66] tracking-wide truncate pr-4">
          {scene.ig}
        </h2>
        <div className="flex gap-2">
          <button
            onClick={() => setAutoPan(!autoPan)}
            className={`px-3 py-1.5 rounded transition flex items-center gap-2 text-xs font-bold uppercase tracking-wide border ${autoPan ? "text-green-400 bg-green-950/30 border-green-800/50" : "text-neutral-500 bg-neutral-900 border-neutral-800 hover:text-neutral-300"}`}
          >
            {autoPan ? <Video size={16} /> : <VideoOff size={16} />}{" "}
          ...
          >
            {layoutData.elements.map((el, idx) => {
              ...
              return (
                <div
                  key={`line-${idx}`}
                  className="absolute w-full border-b border-dashed border-green-900/30"
                  style={{ top: el.shiftedY + 40 }}
                />
              );
            })}
            {layoutData.boxes.map((box) => {
              const isVisible = box.elementIndices.some(
                (idx) => elementStates[idx]?.visible,
              );
              ...
              return (
                ...
                >
                  <div
                    onClick={() =>
                      setCollapsedBoxes((p) => ({ ...p, [box.id]: !p[box.id] }))
                    }
                  ...
                  >
                    <span className="text-sm font-bold text-[#60a5fa] tracking-wider font-mono">
                      apart
                    </span>
                    <span className="text-xs text-[#93c5fd] font-bold flex items-center gap-1 bg-blue-950/50 px-2 py-1 rounded">
                      ...
                      {box.collapsed ? "Expandir" : "Ocultar"}
                    </span>
                  </div>
                </motion.div>
              );
            ...
            {scene.cont.map((el, idx) => {
              ...
              const renderData = {
                ...el,
                x: processedEl.x,
                y: processedEl.shiftedY,
              };
              const renderState = {
                ...state,
                visible: state.visible && !processedEl.isHiddenByBox,
              };
            ...
            >
              <div className="flex justify-between items-center p-4 border-b border-neutral-800">
                <h3 className="text-white font-bold flex items-center gap-2">
                  <BookOpen size={18} className="text-[#00ff66]" /> Recursos
                </h3>
                <button
      ...
      <div className="bg-[#0a0a0a] p-4 border-t border-neutral-800 flex items-center justify-center gap-6 z-20 shrink-0">
        <button
          ...
          className="p-3 text-neutral-500 hover:text-[#00ff66] rounded-full hover:bg-neutral-900 transition"
        >
          <RefreshCw size={20} />
        </button>
        ...
        <button
          onClick={togglePlay}
          className={`flex items-center gap-3 px-8 py-3 rounded-full font-bold text-black shadow-[0_0_15px_rgba(0,255,102,0.3)] transition-all hover:scale-105 active:scale-95 ${isPlaying ? "bg-amber-500 shadow-amber-500/30" : "bg-[#00ff66]"}`}
        >
          {isPlaying ? (
            <>
              <Pause fill="black" size={20} /> Pausar
            </>
          ) : (
            <>
              <Play fill="black" size={20} />{" "}
              {currentStepIdx === -1 ? "Comenzar" : "Continuar"}
            </>
          )}
        </button>
        <button
          ...
          className="p-3 text-neutral-500 hover:text-[#00ff66] hover:bg-neutral-900 rounded-full transition disabled:opacity-30"
        >
          <SkipForward size={24} />
        </button>
      </div>
          ...
          <div className="absolute inset-0 z-[100] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
            ...
            >
              <h3 className="text-white text-lg font-bold mb-2 flex items-center gap-2">
                <Sparkles className="text-amber-500" size={20} /> ¿Qué no
                entendiste?
              </h3>
              <p className="text-neutral-400 text-sm mb-4">
                Cuéntame qué parte de este paso te confunde para que la IA se
                enfoque en ello.
              </p>
              <textarea
                autoFocus
